...
Each plot style can by modified to support particular colors or line style options. 
Currently these modifications are supported only by the underlying gnuplot engine;
future versions of PDL::Graphics::Gnuplot may support parsing them directly.  The 
modifier options, where present, should be interpolated into the "with" specifier string.
For example, to plot a blue line you can use C<<with=>'lines linecolor rgb "blue"'>> instead
of the more terse C<<with=>'lines'>>.  To match the autogenerated style of a particular
line you can use C<<linestyle>> or C<<ls>> as a modifier.  Please see the gnuplot
